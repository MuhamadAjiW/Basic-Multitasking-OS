...
int main(int argc, char *argv[]) {
    ...
    // Read storage into memory, requiring 64 MB memory
    image_storage     = malloc(64*1024*1024);
    file_buffer       = malloc(64*1024*1024);
    FILE *fptr        = fopen(argv[3], "r");
    fread(image_storage, 64*1024*1024, 1, fptr);
    fclose(fptr);

    // Read target file, assuming file is less than 64 MiB
    ...
    if (fptr_target == NULL){
        ...
        printf("File found!\n");
        fread(file_buffer, 64*1024*1024, 1, fptr_target);
    ...
    fwrite(image_storage, 64*1024*1024, 1, fptr);
    fclose(fptr);
