...
void rm(uint32_t currentCluster) {
    int length = sh_parser.word_count;

    if (length >= 2) {
        if (is_directorypath_valid(sh_parser.content[length - 1], currentCluster)) {
            // TODO [minor]: check dir kosong / ngga
            uint8_t isEmptyDir = 0; 
            if (strcmp(sh_parser.content[1],"-r") != 0 && !isEmptyDir) { // flag -r selalu di tengah
                print("\nrm: cannot remove '");
                print(sh_parser.content[length - 1]);
...
void cp(uint32_t currentCluster) {
    ...
    if (len >= 3) { // cp minimal len cmd 3
        ...
        for (int i = 1; i < len - 1; i++) {
            isdir = is_directorypath_valid(sh_parser.content[i], currentCluster);
            isfile = is_filepath_valid(sh_parser.content[i], currentCluster);
            if (strcmp(sh_parser.content[i], "-r") == 0) {
                hasR = 1;
            }
            else if (isfile) {
                srcs[nSrc] = path_to_file_request(sh_parser.content[i], currentCluster);
                isFile[nSrc] = 1;
                nSrc++;
            }
            else if (isdir) {
                srcs[nSrc] = path_to_dir_request(sh_parser.content[i], currentCluster);
                isFile[nSrc] = 0;
                hasDir = 1;
                nSrc++;
            }
        ...
        // cek apakah dest ada
        isdir = is_directorypath_valid(sh_parser.content[len - 1], currentCluster);
        isfile = is_filepath_valid(sh_parser.content[len - 1], currentCluster);
            ...
            if (isfile) { // aman
                ...
                if (hasR && hasDir) {
                    ...
                    if(!is_filename(sh_parser.content[len - 1])) {
                        uint8_t status = copy_create_folders(sh_parser.content[len - 1], currentCluster, 0);
                        if(status != 0) return;
                        dest.parent_cluster_number = path_to_cluster(sh_parser.content[len - 1], currentCluster);
                        memcpy(dest.name, src.name, 8);
                        memcpy(dest.ext, src.ext, 3);
                    }
                    else{
                        uint8_t status = copy_create_folders(sh_parser.content[len - 1], currentCluster, 1);
                        if(status != 0) return;
                        dest = path_to_file_request(sh_parser.content[len - 1], currentCluster);
                    }
                    copy1File(src, dest);
                }
            }
...
void exec(FAT32DriverRequest* req, char** args){
    syscall(SYSCALL_TASK_START, (uint32_t) req, (uint32_t) args, 0);
}
