...
void str_concat(string_t* mainstring, string_t substring){
    mainstring-> content = realloc(mainstring->content, mainstring->len + substring.len + 1);
...
void str_insertc(string_t* mainstring, char c, uint32_t loc){
    mainstring-> content = realloc(mainstring->content, mainstring->len + 2);
...
char str_remove(string_t* mainstring, uint32_t loc){
    ...
    mainstring->content[mainstring->len - 1] = 0;
    mainstring->content = realloc(mainstring->content, mainstring->len);
    mainstring->len--;
