...
struct FAT32FileReader read(struct FAT32DriverRequest request){
    struct FAT32FileReader retval = {0};
    if(request.parent_cluster_number < 2){
    ...
    if (self.directory == 1)
    {
        ...
        output = (struct ClusterBuffer*) kmalloc (sizeof(struct ClusterBuffer) * cluster_count);        
...
struct FAT32DirectoryReader read_directory(struct FAT32DriverRequest request){
    struct FAT32DirectoryReader retval = {0};
    if(request.parent_cluster_number < 2){
        retval.content = (void*) 2;
        return retval;
    }
    ...
    if (self.directory != 1)
    {
        ...
        output = (struct DirectoryTable*) kmalloc (sizeof(struct DirectoryTable) * cluster_count);        
...
uint8_t load(struct FAT32DriverRequest request){
    ...
    struct DirectoryEntry self = get_self_entry(request);
    if (memcmp(&self, &emptyEntry, sizeof(struct DirectoryEntry)) == 0){
        return 1;
    }
